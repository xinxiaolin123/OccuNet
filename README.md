...
|-- assets/
|   |-- occunet_logo.png
|   `-- artifact_examples.png
|-- OccuNet.yaml
|-- artifact_simulation_demo.py
...
| `artifact_simulation_demo.py` | Deterministic demonstration of the 14 physics-informed radiographic artifact families. |
...
| `Test_radiograph.png` | Example de-identified radiograph used by the artifact simulation demonstration. |
| `assets/artifact_examples.png` | GitHub-ready 3 x 5 comparison of the original radiograph and 14 single-artifact examples. |
...
Stage 1 adapts the core contrastive representation-learning formulation of [SimCLR](https://proceedings.mlr.press/v119/chen20j.html) to pelvic and hip radiographs.

For each radiograph, the original image and a physics-informed artifact-augmented view form a positive pair, while views originating from other radiographs in the same batch serve as negatives. Both views are encoded by the YOLOv12-family backbone. The pooled features are passed through a two-layer multilayer perceptron projection head, L2-normalized, and optimized using the normalized temperature-scaled cross-entropy (NT-Xent) loss.

Unlike canonical SimCLR, which constructs two views using general-purpose stochastic augmentations, OccuNet uses an original radiograph and a physics-informed artifact-augmented counterpart to encourage invariance to acquisition-, detector-, and postprocessing-related degradation while preserving clinically relevant femoral-neck anatomy.

After Stage 1 pretraining, the projection head is discarded, and the pretrained backbone weights are transferred to Stage 2 detector fine-tuning.
...
## Physics-informed artifact simulation

Stage 1 pairs each original radiograph with an artifact-augmented view to encourage artifact-invariant representation learning. The repository includes `artifact_simulation_demo.py`, a deterministic visualization utility for the 14 physics-informed artifact families described in the Supplementary Materials:

- dead pixels, electrical interference, mechanical scratches, power fluctuation, and dust spots;
- quantum noise, grid artifact, sensor saturation, and cable shadows;
- electromagnetic interference, detector fault, compression artifacts, window/level variation, and motion blur.

The demonstration applies each artifact individually to the same de-identified AP pelvic radiograph. Fixed random seeds make the outputs reproducible and allow direct visual comparison. These single-effect examples are provided for methodological illustration; the training pipeline used the class-conditional pos-like and neg-like sampling strategies and, when applicable, combinations of effects described in the Supplementary Materials.

<p align="center">
  <img src="assets/artifact_examples.png" alt="Original AP pelvic radiograph and 14 physics-informed artifact examples" width="100%">
</p>

<p align="center">
  <em>
    Original image and 14 physics-informed artifact families applied individually using fixed random seeds. The examples illustrate artifact appearance and are not a patient-level performance result or a complete representation of the training-time sampling distribution.
  </em>
</p>

### Generate the artifact examples

Place `artifact_simulation_demo.py` and `Test_radiograph.png` in the repository root, then run:

```bash
python artifact_simulation_demo.py
```

The default output directory is `artifact_simulation_output/` next to the input image. It contains:

- `00_original.png`;
- 14 individually generated artifact images;
- `artifact_comparison_3x5.png`, a 300-dpi archival comparison; and
- `artifact_examples.png`, a smaller GitHub-ready comparison.

The source image, output directory, shared intensity multiplier, and base random seed can be specified explicitly:

```bash
python artifact_simulation_demo.py \
  --input Test_radiograph.png \
  --output-dir artifact_simulation_output \
  --intensity-factor 1.0 \
  --seed 2026
```

The script reads the input radiograph in grayscale, validates the path, writes lossless PNG outputs, and supports Unicode paths. It does not perform dataset splitting, label processing, model training, or patient-level data handling.
...
- Example de-identified radiograph and artifact comparison for demonstration
...
- The artifact comparison shows fixed-seed, single-effect examples and should not be interpreted as the full class-conditional training distribution.
